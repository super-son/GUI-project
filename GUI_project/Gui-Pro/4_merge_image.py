...
def browse_dest_path():
    ...
    if folder_selceted is None : #사용자가 취소를 누를 때
        return
...
def merge_image():
    ...
    # Image에서 size[0] : width, size[1] : height을 가짐
    ...
    # 프로그레스바 연동
    for idx, img in enumerate(images): #enumerate는 image원소들의 인덱스와 내용을 순서대로 가져온다
        result_img.paste(img,(0,y_offset)) 
        y_offset += img.size[1] 
        
        progress = (idx + 1) / len(images) * 100 #실제 퍼센트 정보 계산, 0을 더하는 이유는 인덱스가 0부터 시작
        p_var.set(progress) # p_var에 만든값을 연동시켜주지
        progress_bar.update()
